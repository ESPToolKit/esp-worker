...
void testDeinitIsSafeBeforeInit() {
	...
	expectEqual(
	    worker.activeWorkers(),
	    static_cast<size_t>(0),
	    "deinit before init should keep worker count at zero"
	);
}
...
void testDeinitIsIdempotent() {
	...
	expectEqual(
	    worker.activeWorkers(),
	    static_cast<size_t>(0),
	    "second deinit should not reintroduce workers"
	);
}
...
void testReinitLifecycleAfterDeinit() {
	...
	expectEqual(
	    ...
	    "first spawn should reserve one worker slot"
	);
	...
	expectEqual(
	    worker.activeWorkers(),
	    static_cast<size_t>(0),
	    "deinit should clear active workers"
	);
	...
	expectEqual(
	    worker.activeWorkers(),
	    static_cast<size_t>(1),
	    "second spawn should reserve one worker slot"
	);
...
void testDeinitReleasesActiveTaskHandles() {
	...
	expectEqual(
	    ...
	    "two workers should be active before teardown"
	);
	expectEqual(
	    test_support::createdTaskCount(),
	    static_cast<size_t>(2),
	    "stubs should observe two created tasks"
	);
	...
	expectEqual(
	    test_support::deletedTaskCount(),
	    static_cast<size_t>(2),
	    "deinit should release all active task handles"
	);
}
...
void testDestructorDelegatesToDeinit() {
...

	{
		...
		expectEqual(
		    worker.activeWorkers(),
		    static_cast<size_t>(1),
		    "one worker should be active before scope exit"
		);
	}
	...
	expectEqual(
	    test_support::deletedTaskCount(),
	    static_cast<size_t>(1),
	    "destructor should deinit and release active worker"
	);
}
